...
import static com.wemap.wemapgl.WeMapMapsPlugin.CREATED;
import static com.wemap.wemapgl.WeMapMapsPlugin.DESTROYED;
import static com.wemap.wemapgl.WeMapMapsPlugin.PAUSED;
import static com.wemap.wemapgl.WeMapMapsPlugin.RESUMED;
import static com.wemap.wemapgl.WeMapMapsPlugin.STARTED;
import static com.wemap.wemapgl.WeMapMapsPlugin.STOPPED;
...
final class WeMapController
  ...
  PlatformView {
  private static final String TAG = "WeMapController";
  ...
  WeMapController(
    ...
    options = options.logoEnabled(false).attributionEnabled(false);
    this.id = id;
    ...
    methodChannel =
      new MethodChannel(registrar.messenger(), "plugins.flutter.io/wemap_maps_" + id);
  ...
  Style.OnStyleLoaded onStyleLoadedCallback = new Style.OnStyleLoaded() {
    ...
    public void onStyleLoaded(@NonNull Style style) {
      WeMapController.this.style = style;
      ...
      mapboxMap.addOnMapClickListener(WeMapController.this);
      mapboxMap.addOnMapLongClickListener(WeMapController.this);
  ...
  private void enableSymbolManager(@NonNull Style style) {
    if (symbolManager == null) {
      ...
      symbolManager.addClickListener(WeMapController.this::onAnnotationClick);
    }
  }
  ...
  private void enableLineManager(@NonNull Style style) {
    if (lineManager == null) {
      ...
      lineManager.addClickListener(WeMapController.this::onAnnotationClick);
    }
  }
  ...
  private void enableCircleManager(@NonNull Style style) {
    if (circleManager == null) {
      ...
      circleManager.addClickListener(WeMapController.this::onAnnotationClick);
    }
  }
  ...
  private void enableFillManager(@NonNull Style style) {
    if (fillManager ==  null) {
      ...
      fillManager.addClickListener(WeMapController.this::onAnnotationClick);
    }
  }
  ...
  public void onMethodCall(MethodCall call, MethodChannel.Result result) {
    switch (call.method) {
      ...
      case "map#addWMSLayer": {
        final String layerID = call.argument("layerID");
        final String wmsURL = call.argument("wmsURL");
        final String tileSize = call.argument("tileSize");
        mapboxMap.getStyle()
            .addSource(new RasterSource(layerID, new TileSet("tileset", wmsURL), Integer.parseInt(tileSize)));
        mapboxMap.getStyle().addLayer(new RasterLayer(layerID + "-layer", layerID));
        break;
      }
      case "map#removeWMSLayer": {
        final String layerID = call.argument("layerID");
        mapboxMap.getStyle().removeLayer(layerID + "-layer");
        mapboxMap.getStyle().removeSource(layerID);
        break;
      }
      case "map#addTraffic": {
        mapboxMap.getStyle().addSource(new RasterSource("traffic-source",
            new TileSet("tileset", "https://mt1.google.com/vt/lyrs=transit,traffic&x={x}&y={y}&z={z}"), 256));
        mapboxMap.getStyle().addLayer(new RasterLayer("traffic-layer", "traffic-source"));
        break;
      }
      case "map#removeTraffic": {
        mapboxMap.getStyle().removeLayer("traffic-layer");
        mapboxMap.getStyle().removeSource("traffic-source");
        break;
      }

      case "map#addSatellite": {
        mapboxMap.getStyle().addSource(new RasterSource("satellite-source",
            new TileSet("tileset", "https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}"), 256));
        mapboxMap.getStyle().addLayer(new RasterLayer("satellite-layer", "satellite-source"));
        break;
      }
      case "map#removeSatellite": {
        mapboxMap.getStyle().removeLayer("satellite-layer");
        mapboxMap.getStyle().removeSource("satellite-source");
        break;
      }
