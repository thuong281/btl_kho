...
class WeMapController: NSObject, FlutterPlatformView, MGLMapViewDelegate, WeMapMapOptionsSink, MGLAnnotationControllerDelegate {
    ...
    init(withFrame frame: CGRect, viewIdentifier viewId: Int64, arguments args: Any?, registrar: FlutterPluginRegistrar) {
        self.trafficLayer = MGLRasterStyleLayer(identifier: "traffic-layer", source: self.trafficSource)
        self.satelliteLayer = MGLRasterStyleLayer(identifier: "satellite-layer", source: self.satelliteSource)
        ...
        channel = FlutterMethodChannel(name: "plugins.flutter.io/wemap_maps_\(viewId)", binaryMessenger: registrar.messenger())
        ...
        if let args = args as? [String: Any] {
            Convert.interpretWeMapMapOptions(options: args["options"], delegate: self)
    ...
    func onMethodCall(methodCall: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "map#addWMSLayer":
            ...
            print(arguments)
            let layerID = arguments["layerID"]
            let wmsURL = arguments["wmsURL"] 
            let tileSize = arguments["tileSize"]
            var WMSSource = MGLRasterTileSource(identifier: layerID!, tileURLTemplates: [wmsURL!], options: [ .tileSize: Int(tileSize!)! ])
            var WMSLayer = MGLRasterStyleLayer(identifier: layerID! + "-layer", source: WMSSource)
        ...
        case "map#removeWMSLayer":
            guard let arguments = methodCall.arguments as? [String: String] else { return }
            let layerID = arguments["layerID"]
            var WMSLayer = self.mapView.style?.layer(withIdentifier: layerID! + "-layer")
            if(WMSLayer != nil){
                self.mapView.style?.removeLayer(WMSLayer!)
            }
            var WMSSource = self.mapView.style?.source(withIdentifier: layerID!)
            if(WMSSource != nil){
        ...
        case "map#update":
            ...
            Convert.interpretWeMapMapOptions(options: arguments["options"], delegate: self)
